...
class Head {
    ...
    function generate($noHead = false, $admin = false) {
        ...
        if ($noHead !== true) {
            ...
            $dat .= '<body>';
            $dat .= '<div id="boardNavDesktop" class="desktop"><span class="boardList">' . get_cached("boardlist") . '</span><div id="navtopright">[<a href="javascript:void(0);" id="settingsWindowLink">Settings</a>][<a href="' . HOME . '" target="_top">' . S_HOME . '</a>]</div></div>';
            $dat .= '<div id="boardNavMobile" class="mobile"></div>';
            $dat .= '<div class="boardBanner">' . $bannerImg . $boardTitle . '</div><hr>' . $ribbon . '<a id="top"></a>';
            $dat .= (ENABLE_ADS) ? "<div class='ads aboveForm'>" . ADS_ABOVEFORM . '<hr></div>' : "";
        } else {
            $dat .= "<body>";
        }
